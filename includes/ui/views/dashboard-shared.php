...
function thisismyurl_shadow_build_explanation_sections_fallback(
	...
	$core_phrase = $is_core
		? __( 'It is part of Shadow by Christopher Ross core coverage, so changes here usually deserve priority.', 'thisismyurl-shadow' )
	...
	$how_tested = sprintf(
		...
		__( 'Shadow by Christopher Ross evaluates %1$s using deterministic rules inside the diagnostic class and records the latest result for dashboard reporting. Confidence for this check is marked as %2$s, which indicates how much direct evidence the test can gather versus inferred signals.', 'thisismyurl-shadow' ),
		$name,
		$confidence
	);
	...
	if ( '' === $signal_text ) {
		$signal_text = __( 'Shadow by Christopher Ross detected a configuration issue in this area.', 'thisismyurl-shadow' );
	}
	...
	$how_to_fix = 'failed' === $status_raw
		? sprintf(
			...
			__( 'What Shadow by Christopher Ross found: %1$s. Recommended next step: %2$s Why Shadow is not auto-fixing this: %3$s After you make this change, wait for the next scheduled run to confirm the result changes to Passed.', 'thisismyurl-shadow' ),
			$signal_text,
			$issue_specific_fix,
...
/**
 * Explain why Shadow by Christopher Ross is not applying a diagnostic fix automatically.
 *
 * @since 0.7056
 * @param string $run_key        Diagnostic run key.
 * @param string $name           Diagnostic display name.
...
function thisismyurl_shadow_get_automation_constraint_reason(
	...
	if ( $has_input_path ) {
		$reason = __( 'the correct value depends on your site intent, naming, or environment details, so Shadow by Christopher Ross needs your input before it can safely write the setting', 'thisismyurl-shadow' );
		if ( '' !== $metadata_note ) {
			$reason .= ' ' . sprintf(
	...
	if ( 'guidance' === $tx_maturity ) {
		$reason = __( 'this change depends on hosting, filesystem access, external services, or operator review, so Shadow by Christopher Ross only provides guidance steps instead of making the change blindly', 'thisismyurl-shadow' );
		if ( '' !== $metadata_note ) {
			$reason .= ' ' . sprintf(
				/* translators: %s: diagnostic metadata note */
				__( 'Diagnostic note: %s.', 'thisismyurl-shadow' ),
	...
	if (
		...
		return __( 'the fix lives at the server or hosting layer, outside normal WordPress settings, so applying it automatically would require assumptions about your infrastructure that Shadow by Christopher Ross cannot safely make', 'thisismyurl-shadow' );
	}
	...
	if (
		...
		return __( 'the fix would involve deleting, relocating, or bulk-editing data, and Shadow by Christopher Ross avoids destructive cleanup unless the change can be reversed safely and validated confidently', 'thisismyurl-shadow' );
	}
	...
	if (
		...
		return __( 'the right fix depends on your content, compliance choices, or plugin stack, so Shadow by Christopher Ross avoids inventing pages, copy, or plugin behavior unless you explicitly provide the inputs', 'thisismyurl-shadow' );
	}
	...
	if (
		...
		return __( 'the change affects user accounts or access policy, so Shadow by Christopher Ross leaves the final decision to the site owner instead of risking an unintended lockout or privilege change', 'thisismyurl-shadow' );
	}
