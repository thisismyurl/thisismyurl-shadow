			...
			'details'      => array(
				...
				'explanation_sections'    => array(
					'summary' => sprintf(
						...
						__( 'Shadow by Christopher Ross found %d expired transient timeout records still present in wp_options. Transients are intended to be temporary, but WordPress only removes many of them lazily when requested, so expired entries can accumulate for long periods on active sites.', 'thisismyurl-shadow' ),
						$count
					),
					'how_wp_shadow_tested' => __( 'Shadow by Christopher Ross queried the environment helper for expired transient timeout counts and evaluated the result against a practical threshold. The check is skipped or naturally low-impact when an external object cache is in place, because transient storage and expiration behavior differ in that configuration.', 'thisismyurl-shadow' ),
